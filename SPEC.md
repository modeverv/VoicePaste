...
| F-06 | グローバルホットキー | アプリのフォーカスに依存しないホットキーでGUIを最前面表示し、録音開始・停止する |
| F-07 | GUI表示 | TUIと同じ状態・暫定テキスト・確定テキストを最前面ウィンドウに表示し、Escで非表示にできる |
...
- 起動時はGUIウィンドウを表示する
- グローバルホットキー押下時はGUIウィンドウを最前面に表示し、録音を開始する
- グローバルホットキー離上時は録音を停止し、文字起こしを開始する
- GUIにフォーカスがある状態で `Esc` を押すと録音を停止し、GUIウィンドウを非表示にする
- GUI非表示中にグローバルホットキーを押すと、再表示して新しい録音を開始する
